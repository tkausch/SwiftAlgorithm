//: # Word Ladder
//:
//: **Question:** Given two words (beginWord and endWord), and a word list, find the length of shortest transformation sequence from beginWord to endWord, such that:
//:
...
func ladderLength(_ beginWord: String,_ endWord: String, _ words: [String]) -> Int {
    
    var letters = [String]()
    for val in UnicodeScalar("a").value...UnicodeScalar("z").value {
        letters.append(String(UnicodeScalar(val)!))
    }
    
    let wordList = Set(words)
    var currentWords = Set<String>()
    var nextWords = Set<String>()
    var visited = Set<String>()
    var length = 1
    
    currentWords.insert(beginWord)
    ...
    while(!currentWords.isEmpty) {
        for word in currentWords {
            if word == endWord {
                ...
                for idx in word.characters.indices {
                    for ch in letters {
                        let nextWord = word.replacingCharacters(in: idx..<word.index(after: idx), with: ch)
                        if wordList.contains(nextWord) && !visited.contains(nextWord) && nextWord != word {
...
//: The runtime of this solution is dependent on the number of words and the size of the words. For each position in a word we try out all letters and check if the word exists in the wordList. This is ineffective and there is a much better way to figure out the neighbors of a word. 

func bucketNamesFor(word: String) -> [String] {
    var bucketNames = [String]()
    for idx in word.characters.indices {
        let key = word.replacingCharacters(in: idx..<word.index(after: idx), with: "_")
        bucketNames.append(key)
    }
    return bucketNames
}

func ladderLength2(_ beginWord: String,_ endWord: String, _ words: [String]) -> Int {
    
    // build word to bucketname map
    var wordToBucketNamesMap = [beginWord: bucketNamesFor(word: beginWord)]
    for word in words {
        wordToBucketNamesMap[word] = bucketNamesFor(word:word)
    }
    
    // generate word buckets
    var allWordBuckets = [String: [String]]()
    for pair in wordToBucketNamesMap {
        for bucketName in pair.value {
            if allWordBuckets[bucketName] == nil  {
                allWordBuckets[bucketName] = [pair.key]
            } else {
                allWordBuckets[bucketName]?.append(pair.key)
            }
        }
    }
    
    // filter all word buckets
    var wordBuckets = [String: [String]]()
    for pair in allWordBuckets {
        if pair.value.count >= 2 {
            wordBuckets[pair.key] = pair.value
        }
    }
    ...
    while(!currentWords.isEmpty) {
        for word in currentWords {
            if word == endWord {
                ...
                let bucketNames = wordToBucketNamesMap[word]!
                for bucketName in bucketNames {
                    if let wordBucket = wordBuckets[bucketName] {
                        for nextWord in wordBucket {
                            if !visited.contains(nextWord) && word != nextWord {
                                nextWords.insert(nextWord)
                            }
                        }
                    }
